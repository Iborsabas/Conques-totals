...
def find_rivers_wrtfile(f,id):
    ...
    if not rivers.has_key(id):
        ...
        if all_floats:
            ...
            f.write(';')
            dure = float(rivers[id][5])*float(rivers[id][3]) + sum(float(rivers[k][5]) * float(rivers[k][3])for k in enders)
            f.write(str(dure/totalcatchment))
            f.write(';')
            srd = float(rivers[id][7])*float(rivers[id][3]) + sum(float(rivers[k][7]) * float(rivers[k][3])for k in enders)
            f.write(str(srd/totalcatchment))
            f.write(';')
            dird = float(rivers[id][9])*float(rivers[id][3]) + sum(float(rivers[k][9]) * float(rivers[k][3])for k in enders)
            f.write(str(dird/totalcatchment))
            f.write(';')
            difd = float(rivers[id][11])*float(rivers[id][3]) + sum(float(rivers[k][11]) * float(rivers[k][3])for k in enders)
